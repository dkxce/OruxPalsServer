...
namespace OruxPals
{
    public class OruxPalsServer
    {
        ...
        private FlightRadarGrabber FlightRadar;
        private int FlightRadarClients = 0;
        ...
        public OruxPalsServer() 
        {
            ...
            FlightRadar = new FlightRadarGrabber();
            FlightRadar.OnAirData += new FlightRadarGrabber.DataEvent(FlightRadar_OnData);
        ...
        private void ClientThread(object param)
        {
            ...
            if ((cd.state == 4) && (cd.FR24))
            {
                FlightRadarClients--;
                if (FlightRadarClients == 0) FlightRadar.Stop();
            };
            cd.client.Close();
            cd.thread.Abort();
        }
        ...
        private bool OnAPRSClient(ClientData cd, string loginstring)
        {
            ...
            if (rm.Success)
            {
                ...
                if (doptext.ToUpper().IndexOf("FR24") > 0)
                {
                    cd.FR24 = true;
                    FlightRadarClients++;
                    if (cd.filter != null)
                    {
                        if (cd.filter._fr24_zoneW > 0)
                            FlightRadar.ScanHeight = FlightRadar.ScanWidth = (float)cd.filter._fr24_zoneW;
                        if (cd.filter._fr24_zoneH > 0)
                            FlightRadar.ScanHeight = (float)cd.filter._fr24_zoneH;
                        if (!String.IsNullOrEmpty(cd.filter._fr24_zone))
                            FlightRadar.ScanZone = cd.filter._fr24_zone;
                        if (cd.filter._fr24_interval > 0)
                            FlightRadar.ScanInterval = (byte)cd.filter._fr24_interval;
                    };
                    if (!FlightRadar.IsActive)
                        FlightRadar.Start();
                };
        ...
        private void OnAPRSData(ClientData cd, string line)
        {
            ...
            if ((b != null) && (b.name != null) && (b.name != String.Empty))
            {
               ...
               if ((b.PositionIsValid)) // if Position Packet send to All (AIS + APRS + Web)
               {
                   ...
                   // IF FR24
                   if ((cd.FR24)) // USER REQUESTS FlightRadar24 DATA // UPDATE LAT & LON TO REQUEST // IF SEVERAL USERS THEN USE LAST SPECIFIED LAT/LON
                   {
                       FlightRadar.ScanFromLat = (float)b.lat;
                       FlightRadar.ScanFromLon = (float)b.lon;
                   };
               }
               ...
               // NARODMON
               if ((b.PositionIsValid) && (cd.filter != null) && (cd.filter.narodmon.Length == 2))
               {
                   NarodMonGet(cd, line, b, cd.filter.narodmon[0], cd.filter.narodmon[1]);
               };
            };
        }

        public struct NarodMonThreadObj
        {
        ...
        private void NarodMonGet(ClientData cd, string line, Buddie b, int radius, int limit)
        {
            if (DateTime.UtcNow.Subtract(cd.lastNarodMon).TotalMinutes < 5) return; // 1 times per 5 minutes

            NarodMonThreadObj nmc = new NarodMonThreadObj();
            nmc.callsign = b.name;
            nmc.client_id = cd.id;
            nmc.lat = b.lat;
            nmc.lon = b.lon;
            ...
            Thread thr = new Thread(NarodMonThreadProc);
            thr.Start(nmc);            
        }
        private void NarodMonThreadProc(object data)
        {
            NarodMonThreadObj nmc = (NarodMonThreadObj)data;
            NarodMonAPI nmapi = new NarodMonAPI();
            NarodMonAPI.Resp_sensorsNearby nb = nmapi.sensorsNearby(nmc.callsign, nmc.lat, nmc.lon, nmc.raidus, "1,2,3,9", nmc.limit, 2000);
            if (nb == null) return;
            List<byte[]> apss = new List<byte[]>();
            foreach (NarodMonAPI.Device d in nb.devices)
                if (d.HasTemp)
                {
                    DateTime dt = UnixTimeStampToDateTime(d.sensup);
                    TimeSpan ts = DateTime.Now.Subtract(dt);
                    if (ts.TotalMinutes < 60)
                    {
                        Buddie bb = new Buddie(0, d.Callsign, d.lat, d.lng, 0, 0);
                        bb.IconSymbol = @"\p";
                        bb.Status = bb.Comment = d.Text.Replace("°", "deg");
                        bb.SetAPRS();
                        apss.Add(bb.APRSData);
                    };
                };
            if (apss.Count == 0) return;                
            ...
            lock (clientList)
                foreach (object obj in clientList.Values)
                {
                    ...
                    if (cd.id == nmc.client_id) client = cd;
                };
            ...
            try
            {
                client.lastNarodMon = DateTime.UtcNow;
                foreach (byte[] packet in apss)
                {
                    client.client.GetStream().Write(packet, 0, packet.Length);
                    client.client.GetStream().Flush();
                };
            }
            catch { };
        }
        public PreloadedObject[] NarodMonGetNearest(double lat, double lon)
        {
            List<PreloadedObject> objs = new List<PreloadedObject>();

            NarodMonAPI nmapi = new NarodMonAPI();
            NarodMonAPI.Resp_sensorsNearby nb = nmapi.sensorsNearby("webuser", lat, lon, 10, "1,2,3,9", 50, 9000);
            if (nb == null) return null;

            foreach (NarodMonAPI.Device d in nb.devices)
            {
                if (!d.HasTemp) continue;
                PreloadedObject po = new PreloadedObject(d.Callsign, @"\p", d.lat, d.lng, d.distance, d.Text.Replace("°", "deg"), "NM");
                objs.Add(po);                
            };
        ...
        private void OnBrowser(ClientData cd, string query, string rxText)
        {
            ...
            HTTPClientSendResponse(cd.client, GetPageHTMLHeader() + GetPageHeader(0, isAdmin) + String.Format(
                ...
                FlightRadar.Parameters + ":\r\n<br/>" +
                (FlightRadar.IsActive ? (FlightRadar.TotalScanCount.ToString() + " updates, last " + FlightRadar.LastUpdate.ToString("HH:mm:ss dd.MM.yyyy") + " (now <a href=\"fr24\">" + (lastPlanes == null ? "0" : lastPlanes.Length.ToString()) + "</a> planes)") : "offline") + "\r\n</div><hr/>" +
                ...
                urlPath,
                rbds + ubds,
                cAPRS.ToString() + "[" + FlightRadarClients.ToString() + "]",
                cFRS,
                fsvc,
                cAPRSr,
                cAIR, cWSW, cWST
        ...
        private void OnView(ClientData cd, string query, string rxText)
        {
            ...
            else if (prf == "near")
            {
                ...
                if (m.Success)
                {
                    ...
                    if (BUDS != null)
                    {
                        PreloadedObject[] no = null;
                        if (ask["what"] == "weather")
                            no = NarodMonGetNearest(lat, lon);
                        else
                            no = BUDS.GetNearest(lat, lon);
            ...
            {
                string PLANES_DATA = "<small style=\"color:maroon;\">" + FlightRadar.Parameters + "\r\n<br/>";
                PLANES_DATA += "Last Error: " + (String.IsNullOrEmpty(FlightRadar.LastError) ? "-no-" : FlightRadar.LastError) + " \r\n<br/>";
                PLANES_DATA += "URL: " + FlightRadar.LastURL + "\r\n<br/>";
                PLANES_DATA += "Updates " + FlightRadar.TotalScanCount.ToString() + ", last: " + FlightRadar.LastUpdate.ToString("HH:mm:ss dd.MM.yyyy") + " \r\n<br/>\r\n<br/>";
                if (!FlightRadar.IsActive)
                    PLANES_DATA += "NO FR24 CLIENTS, INACTIVE\r\n<br/>";
                PLANES_DATA += "</small>";

                lpMutex.WaitOne();
                if ((lastPlanes == null) || (lastPlanes.Length == 0))
                    PLANES_DATA += "<b>NO PLANES DATA</b>\r\n<br/>";
                else
                {
                    PLANES_DATA += "<b>" + lastPlanes.Length.ToString() + " PLANES FOR " + FlightRadarClients.ToString() + " USER(S):</b>\r\n<br/>";
                    foreach (AirCraft plane in lastPlanes)
                        PLANES_DATA += " - " + plane.ToString() + "\r\n<br/>";
                };
                lpMutex.ReleaseMutex();

                HTTPClientSendResponse(cd.client, PLANES_DATA);
            }
            else
            {
                if (ptf == "") // MAP VIEW
        ...
        private AirCraft[] lastPlanes;
        private void FlightRadar_OnData(AirCraft[] AirShips, DateTime update)
        {
            lpMutex.WaitOne();
            lastPlanes = new AirCraft[0];
            lpMutex.ReleaseMutex();

            if (AirShips == null) return;
            if (AirShips.Length == 0) return;

            lpMutex.WaitOne();
            lastPlanes = (AirCraft[])AirShips.Clone(); // To HTTP Out
            lpMutex.ReleaseMutex();
            
            AirCraft[] ships = (AirCraft[])AirShips.Clone();

            List<byte[]> b_data = new List<byte[]>();
            List<ushort> a_speed = new List<ushort>();
            List<string> b_comm = new List<string>();

            foreach (AirCraft ship in ships)
            {
                try
                {
                    Buddie b = new Buddie(7, ship.CallSign, ship.Lat, ship.Lon, 0, ship.Hdg);
                    b.speed = (short)(ship.Spd * 1.852); // knots to kmph
                    b.IconSymbol = "/'";
                    if ((ship.Alt < 5) || (b.speed < 5))
                        b.IconSymbol = @"//";
                    else if (ship.Alt < 1300) // 400 meters
                        b.IconSymbol = @"/g";
                    else if (ship.Alt > 29500) // more than 9 km
                        b.IconSymbol = @"\^";
                    b.Comment = ship.ShortHint;
                    b.SetAPRS();                    
                    b_data.Add(b.APRSData);
                    a_speed.Add(ship.Spd);
                    b_comm.Add(b.Comment);                    
                }
                catch { };                
            };
            
            if(b_data.Count > 0)
                BroadcastFR24(b_data, a_speed.ToArray(), b_comm.ToArray());
        }
